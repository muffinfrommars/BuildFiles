
aMountedClassTable .block ; 88/8000
	.byte Cavalier
	.byte LanceKnight
	.byte BowKnight
	.byte BowKnightF
	.byte AxeKnight
	.byte SwordKnight
	.byte Troubadour
	.byte KnightLord
	.byte DukeKnight
	.byte MasterKnight
	.byte MasterKnightF
	.byte Paladin
	.byte PaladinF
	.byte ArchKnight
	.byte ArchKnightF
	.byte Ranger
	.byte MageKnight
	.byte MageKnightF
	.byte GreatKnight
	.byte PegasusKnight
	.byte TrueArcher ; FalconKnight
	.byte Dracorider
	.byte DracoriderF
	.byte Dracoknight
	.byte DracoknightF
	.byte DragonMaster
	.byte DragonMasterF
	.byte PegasusRider
	.bend

aDismountedClassTable .block ; 88/801C
	.byte CavalierDismounted
	.byte LanceKnightDismounted
	.byte BowKnightDismounted
	.byte BowKnightFDismounted
	.byte AxeKnightDismounted
	.byte SwordKnightDismounted
	.byte TroubadourDismounted
	.byte KnightLordDismounted
	.byte DukeKnightDismounted
	.byte MasterKnightDismounted
	.byte MasterKnightFDismounted
	.byte PaladinDismounted
	.byte PaladinFDismounted
	.byte ArchKnightDismounted
	.byte ArchKnightFDismounted
	.byte RangerDismounted
	.byte MageKnightDismounted
	.byte MageKnightFDismounted
	.byte GreatKnightDismounted
	.byte PegasusKnightDismounted
	.byte TrueArcherDismounted ; FalconKnightDismounted
	.byte DracoriderDismounted
	.byte DracoriderFDismounted
	.byte DracoknightDismounted
	.byte DracoknightFDismounted
	.byte DragonMasterDismounted
	.byte DragonMasterFDismounted
	.byte PegasusRiderDismounted
	.bend
